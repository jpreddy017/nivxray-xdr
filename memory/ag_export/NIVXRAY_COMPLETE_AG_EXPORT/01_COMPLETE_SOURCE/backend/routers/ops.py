...
async def decode_smart(body: AutoIn, user=Depends(get_current_user)):
    ...
    # ── Universal Decoded Intelligence & Stop Reason Exposure ─────────────
    # Ensures the analyst UI and API consumers receive complete multi-stage
    # evidence, explicit stop reasons, and standardized per-stage aliases.
    try:
        _stop_reason = (
            result.get("stop_reason")
            or (_canonical_artifact.iedde_trace or {}).get("stop_reason")
            or det.get("stop_reason")
            or det.get("stopped_reason")
            or (result.get("investigation") or {}).get("rte", {}).get("stop_reason")
            or ("terminal_plaintext_reached" if len(result.get("trace") or []) > 0 else "no_transformation_identified")
        )
        result["stop_reason"] = _stop_reason
        result["stopped_reason"] = _stop_reason

        # Standardize trace aliases on every stage
        _norm_trace = []
        for _idx, _t in enumerate(result.get("trace") or []):
            _step = dict(_t)
            _op = _step.get("op") or _step.get("decoder") or f"stage_{_idx+1}"
            _why = _step.get("why") or _step.get("reason") or _step.get("why_selected") or ""
            _prev = _step.get("preview") or _step.get("output_preview") or ""
            _in_l = _step.get("in_len") or _step.get("input_length") or len(body.input or "")
            _out_l = _step.get("out_len") or _step.get("output_length") or len(_prev)
            _step["sequence"] = _idx + 1
            _step["op"] = _op
            _step["decoder"] = _op
            _step["why"] = _why
            _step["reason"] = _why
            _step["why_selected"] = _why
            _step["preview"] = _prev
            _step["output_preview"] = _prev
            _step["output_payload"] = _step.get("output_payload") or _step.get("output") or _prev
            _step["in_len"] = _in_l
            _step["input_length"] = _in_l
            _step["out_len"] = _out_l
            _step["output_length"] = _out_l
            _step["status"] = _step.get("status") or ("failed" if "error" in _step else "success")
            _step["duration_ms"] = _step.get("duration_ms") or _step.get("exec_ms", 0)
            _step["exec_ms"] = _step["duration_ms"]
            _step["stop_reason"] = _stop_reason
            _norm_trace.append(_step)
        result["trace"] = _norm_trace

        if "decoded_intelligence" not in result or not result["decoded_intelligence"]:
            result["decoded_intelligence"] = {
                "raw_command": body.input or "",
                "effective_payload": result.get("output_raw") or result.get("output") or "",
                "stages": _norm_trace,
                ...
                "semantic_understanding": result.get("semantic") or {},
                "provenance": {
                    "engine": result.get("engine") or "ddo_pipeline",
                    "stages_count": len(_norm_trace),
                    "original_length": len(body.input or ""),
                    "final_length": len(result.get("output_raw") or result.get("output") or ""),
                },
            }
    except Exception:
        log.exception("Failed to attach decoded_intelligence/stop_reason in /decode/smart")
