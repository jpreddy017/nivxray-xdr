/**
 * Canonical MITRE ATT&CK Enterprise Matrix · v16 (Oct 2024)
 * ---------------------------------------------------------
 * Full top-level technique taxonomy for the 14 Enterprise tactics —
 * complete, not a trimmed subset.  Sub-techniques are surfaced via
 * cell click; the grid intentionally shows the parent technique so
 * the kill-chain readout stays legible.
 *
 * Owner-locked guarantees:
 *   • NOT AN ENGINE.  This module is a pure lookup table.
 *   • Detection counts are rolled up from authoritative NivXRay
 *     incident evidence at render time.  Heat is real detections,
 *     NEVER a fabricated risk score.
 *   • Source of truth: https://attack.mitre.org/versions/v16/
 */
...
// Deterministic mapping of the rule_id prefixes NivXRay's Stage-2
// verdict emits to MITRE technique ids.  Real detections roll up
// here; unmapped rules simply don't contribute to the heatmap and
// the coverage-gap panel calls that out honestly.
export const RULE_TO_TECHNIQUE = {
  "CMD-OBFUSCATION":            "T1027",
  "PROC-SUSPICIOUS-PARENT":     "T1055",
  "FILE-DROP-EXECUTABLE":       "T1105",
  "NETWORK-SUSPICIOUS":         "T1071",
  "LSASS-ACCESS":               "T1003",
  "SUSPICIOUS-POWERSHELL":      "T1059",
  "SCHEDULED-TASK-CREATE":      "T1053",
  ...
  "MITRE-RESOURCE-DEVELOPMENT": "T1588",
};

// Flat index by technique id → { id, name, tactic }.  Multiple
// tactic memberships (e.g. T1078 Valid Accounts appears in four
// tactics) are recorded once — the last-seen tactic wins.  The UI
// tags each cell with its column tactic explicitly so the index
// isn't used for tactic attribution.
export const TECHNIQUE_INDEX = (() => {
  const out = {};
  for (const [tactic, arr] of Object.entries(TECHNIQUES_BY_TACTIC)) {
    for (const t of arr) out[t.id] = { ...t, tactic };
  }
  return out;
})();

// Distinct techniques (deduped across tactics), used for the KPI header.
export const DISTINCT_TECHNIQUE_IDS = (() => {
  const s = new Set();
  for (const arr of Object.values(TECHNIQUES_BY_TACTIC)) {
