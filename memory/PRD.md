...
> **2026-02-15 · P0d-A · Frontend wiring of Deterministic Analyst Brief in Prev-Mode — 🟢 CLOSED** — Owner-authorised frontend-only fix (`frontend/src/pages/WorkspacePage.jsx`, +58 LOC additive) that mounts `InvestigationSummaryPanel` directly below `AnalystNarrativePanel`. Two additions: (1) new `sessionSnapshot` React state and a scoped `useEffect` that calls `/api/session/from-investigation` whenever `investigationObject` acquires meaningful evidence — `acquired_document.ok`, or any of `commands` / `artifacts` / `incident.iocs` / `report_extraction.body_artifacts`, or the top-level `iocs` bucket (atomic-IOC-URL path — the exact defect for the 108-byte SystemWeakness URL Analyst Paste); (2) render block guarded on `sessionSnapshot?.summary_narrative` with `PanelErrorBoundary` and `data-testid="workspace-investigation-summary-wrap"`. Zero backend / router / IUE / IDA / DIE / provenance / XOR-decoder / M0e-plumbing changes. Screenshot verification with `admin@nivxray.com` on the exact 108-byte SystemWeakness URL Analyst Paste: all 9 brief cards render (`summary-executive` / `summary-analyst` / `summary-behaviours` / `summary-intent` / `summary-impact` / `summary-mitre` / `summary-iocs` / `summary-recs` / `summary-evidence` — every test-id FOUND). Executive Summary shows Overall Risk: LOW · Confidence: 30%; Evidence Confidence card shows Commands 0/0 · MITRE 0 · IOCs 0 · Threat Intel pending · Completeness 0%. Note: the IOC-count-0-vs-1 gap is a backend `summary_narrative.ioc_intelligence` projection concern (top-level `iocs.url` isn't lifted into `ioc_intelligence`) — LOCKED per SystemWeakness URL constraint; owner explicitly ruled it out of P0d-A scope. Backend untouched (`git status` confirms only `WorkspacePage.jsx` modified). Frontend `craco start` recompiled cleanly. Analyst-Paste Prev-Mode now surfaces the 9-card brief without navigation to `/workspace/session/:id`.
>
