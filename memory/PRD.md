...
### ✅ RC3.8 · Case Library + Sigma Export + Documents UI + Family-Op registration  ← DELIVERED (2026-07-20)
    ...
    - **📂 Documents Page** (`pages/DocumentsPage.jsx` + route `/documents` + nav pill `DOCUMENTS`): multi-format uploader (PDF/DOC/DOCX/CSV/XLS/XLSX/JSON/TXT/EML/HTML/MD/YAML/PS1/BAT/SH/PY/JS/VBS/LOG, 25 MB cap). Per-file actions: PREVIEW (in-browser modal), DECODE (extracts text and pipes through `/decode/smart`), INGEST (JSON → `ir_export_to_fixture.py`), Download, Delete. Backend router `documents.py` is GridFS-backed.
    - **🎨 Family-op registration** (`operations.py`): `family-meterpreter`, `family-cobaltstrike`, `family-xworm`, `family-redline`, `family-formbook`, `family-njrat`, `family-emotet`, `crypto-detect`, `crypto-key-required` are now first-class no-op annotators — trace no longer shows red `Unknown operation` cosmetic warnings for terminal fingerprint layers.
