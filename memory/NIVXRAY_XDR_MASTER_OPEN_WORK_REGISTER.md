...
| B-07 | DSM coverage → canonical evidence → deterministic detection proof | P1 | **SECURITY + POWERSHELL DONE (2026-06) · REST OPEN** | B-01..B-06 | no fabricated telemetry | `evtx_xml.py` decodes rendered EVTX XML once at ingest (`_document_for_pipeline`, outcome recorded under `_nivx.evtx_decode`) — without it NO Windows DSM could ever claim a W2-1 delivery. `windows_security_dsm` widened to 4648/4672/4720/4726/4732/4776/4698/1102 (1102 from `UserData`); new `windows_powershell_dsm` (4103/4104/4105/4106 + classic 400/403/500/501/600/800). Both declare OBSERVATION_TIME, never activity. 33 new tests + D12 guard. Report: `memory/W2-1B_WINDOWS_CHANNEL_DSM_REPORT.md` | Defender → Task Scheduler → WMI → AppLocker → System/Application |
| B-08 | Channel coverage (21 channels architected) | P1 | **COLLECTION IMPLEMENTED / ANALYSIS PARTIAL** | B-01 | — | Sysmon · Security · PowerShell (×2) · Defender · Task Scheduler · WMI · AppLocker · Code Integrity · RDP (×2) · WinRM · DNS Client · Windows Firewall · SMB · Directory Service · DNS Server audit · System · Application · BitLocker · Windows Update. Each carries a security-value rating. **Normalized: Sysmon, Security, both PowerShell channels** | Analysis support per B-07 |
| B-15 | Declared-source routing for the Windows channels | P1 | **DONE (2026-06)** | B-07 | a declaration may never widen what a collector is authorized to send | `windows_security` / `windows_powershell` / `powershell` / `microsoft_defender` registered as ALIASES in `source_routing.SOURCE_ALIASES`, resolving to the ONE catalog key permitted to interpret them (`windows-security-evd`, `windows-powershell-evd`, `windows-defender-evd`). Before this they were refused `UNSUPPORTED_SOURCE`. A Security payload declared as PowerShell is refused `SOURCE_FORMAT_MISMATCH` — asserted | Re-verify on real endpoint (W2-R3) |
| B-16 | Microsoft Defender channel evidence | P1 | **DONE (2026-06)** | B-07 | Defender's verdict is SOURCE evidence, never a NivXRay verdict | `windows_defender_dsm.py` · detection (1006/1015/1116), response (1007/1008/1009/1117/1118/1119) and posture (1150/1151/5001/5004/5007/5010/5012) families. `additional_fields.vendor_verdict` carries Microsoft's threat/severity/action verbatim with an authority note; no verdict written, no incident promoted, Command Intelligence not invoked. `Detection Time` = ACTIVITY_TIME, `TimeCreated` = observation | Detection content targeting endpoint-protection evidence |
| G-01 | Windows channel truth model + admin console | P1 | **DONE (2026-06)** | B-07, B-16 | read-only; tenant-scoped via `resolve_principal` | `services/windows_channel_truth.py` publishes FIVE independent dimensions (Collection · Parsing · Normalization · Detection capability · Detection activity) with NO composite health verdict. Capability computed from deployed content, never from firings. `/api/xdr/windows/*` + `/xdr/data-sources/windows/:tab`. Device identity: `origin_computer` = evidence of origin, `canonical_device_id` NOT ESTABLISHED, EDR association NOT ESTABLISHED (never UNENROLLED). Report `memory/W2-1C_WINDOWS_TRUTH_AND_EVENT_EXPLORER.md` | Publish collector bookmark/queue position (W2 C-4) |
| G-02 | Coverage Impact · POTENTIAL vs EFFECTIVE | P1 | **DONE (2026-06)** | G-01 | a coverage claim must cite the evidence it rests on | `coverage_impact()` publishes two independent claims. POTENTIAL = deployed content that could use the source (declared fields). EFFECTIVE = source receiving · parser · normalization · required fields MEASURED in real evidence · rule deployed · rule applicable. Neither requires a firing. Itemised prerequisites with the exact blocker; evidence gaps kept apart from content gaps; a rule declaring no required fields is BLOCKED, not effective. ATT&CK only where content carries an authoritative mapping. `/api/xdr/windows/coverage` + `WindowsCoverage.jsx` with the citation pane. Benchmark recorded in `memory/ENGINEERING_STANDARD_INDUSTRY_BENCHMARK.md` | Detection content for Security/PowerShell/Defender evidence |
| H-01 | Event Explorer · authoritative estate-wide event surface | P1 | **DONE (2026-06)** | G-01 | `evidence.read`; canonical evidence only | `/api/xdr/events/search · /facets · /{id}` over `xdr_canonical_evidence` — source-agnostic, so every DSM's output appears under one contract. `/xdr/events` dense table + 8-tab pane over the explicit chain Raw → Parsed → Normalized → Canonical → Detection → Incident, each stage with state + evidence ref. Raw XML immutable. No fixtures | Saved hunts · time-range picker · export |
