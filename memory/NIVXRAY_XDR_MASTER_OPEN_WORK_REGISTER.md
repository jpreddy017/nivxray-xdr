...
| B-15 | Declared-source routing for the Windows channels | P1 | **DONE (2026-06)** | B-07 | a declaration may never widen what a collector is authorized to send | `windows_security` / `windows_powershell` / `powershell` registered as ALIASES in `source_routing.SOURCE_ALIASES`, resolving to the ONE catalog key permitted to interpret them (`windows-security-evd`, `windows-powershell-evd`). Before this they were refused `UNSUPPORTED_SOURCE`. A Security payload declared as PowerShell is refused `SOURCE_FORMAT_MISMATCH` — asserted | Re-verify on real endpoint (W2-R3) |
