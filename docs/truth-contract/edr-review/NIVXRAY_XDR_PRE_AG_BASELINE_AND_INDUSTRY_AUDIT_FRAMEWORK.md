...
| ~~**DEV-3**~~ | ~~**Two case stores.** `workspace_cases` (484) is the analyst-facing legacy store; `xdr_incidents` (1) is the canonical pipeline output; `v2_cases` (35) is a third.~~ **SUPERSEDED 2026-09-05 by DEV-3′** — see `NIVXRAY_XDR_P0_1_CASE_STORE_RECONCILIATION.md` §0/§7. The canonical pipeline writes incidents into **`workspace_cases`** (`detection_content/xdr_incident.py:26,114`), which three source files independently declare authoritative. `xdr_incidents` is a **Cortex/BYO-EDR vendor mirror** (2 readers, 1 doc, separate ID namespace); `v2_cases` is **deliberately isolated** (`v2/case_engine/schema.py:8`). | Mongo counts + writer trace | ~~P0~~ → **withdrawn** |
| **DEV-3′** | **One authoritative store, three undiscriminated document shapes.** `workspace_cases` (484) holds 198 XDR pipeline incidents (`xdr_pipeline` present), 209 analysis/SSOT cases (`ssot` present), 0 overlap, and 77 with neither — with **no explicit `doc_type` discriminator**; each of 48 reader modules infers type implicitly. | `NIVXRAY_XDR_P0_1_CASE_STORE_RECONCILIATION.md` §2, §4 | **P1** (downgraded from P0 — no ambiguity of authority) |
| **DEV-8** | **`verdict_stage2` is read by three projections and written on zero documents.** `routers/edr.py:3-8` derives `/api/edr/detections` from `workspace_cases.verdict_stage2.evidence[]`; live count of docs carrying `verdict_stage2` = **0 of 484**. Pipeline incidents write `verdict_card` instead (`xdr_incident.py:88-93`). | live `count_documents` | **P1** — `/api/edr/detections` is structurally always empty |
| **DEV-9** | **The AG "unified" DSM registry is dead in production.** `TELEMETRY_DSM_REGISTRY` (`telemetry/registry.py:39`) has **zero** production consumers (tests only), does not contain `SysmonDSM`, and its `register_dsm()` is never called. Production uses `DSM_REGISTRY` (`xdr_pipeline.py:74`) exclusively. | `NIVXRAY_XDR_P0_2_DSM_REGISTRY_OWNERSHIP_CONFLICT_MAP.md` §1 | **P0** (refines DEV-2) |
...
3. ~~**Decide the case-store single source of truth (DEV-3).**~~ **REVISED 2026-09-05 (DEV-3′):** the SSOT is **already** `workspace_cases`, declared consistently in `xdr_incident.py:5-7`, `routers/incidents.py:1-7` and `v2/case_engine/schema.py:8`, with exactly **one** incident-creating writer. No migration is required. The remaining work is **XS**: add an explicit `doc_type` discriminator, publish the contract for the 48 readers, and resolve DEV-8 (`verdict_stage2` read-but-never-written). See `NIVXRAY_XDR_P0_1_CASE_STORE_RECONCILIATION.md` §8.
