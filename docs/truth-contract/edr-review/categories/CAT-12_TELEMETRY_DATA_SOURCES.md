...
| **Two competing DSM registries** | **P0** | `xdr_pipeline.DSMRegistry` (`:51-74`) hard-codes `SnortEveDSM` then *try-imports* the telemetry package with `except Exception: pass` at `:59` and `:65`; `telemetry/registry.TelemetryDSMRegistry` (`:14-39`) is a second registry with its own `register_dsm()`. **A DSM registered in one is invisible to the other, and load failures are swallowed silently.** Master DEV-2 |
