...
| P2-06 | P2 ⭐⭐⭐⭐ | **Semantic Correlation Engine (CRE++)** — replaces event-based correlation (`Event A + Event B`) with evidence-chain correlation: `PowerShell downloader + DNS lookup + Downloaded file + SHA256 in VT + Registry Run Key → Attack Story`. Produces the ordered Attack Story that the Story lens renders. Deterministic — no LLM in the correlation path. | ⏳ Pending | Follows P2-05d. |
