...
| P2-05 | P2 ⭐⭐⭐⭐⭐ | **IDI Engine — Investigation Document Intelligence.** Ingestion adapter layer that turns Cisco XDR / CrowdStrike / Defender / QRadar / Splunk / Sysmon / Windows Event / etc. into the CIO. NOT a parallel pipeline — adapters feed the shared verdict engine and CIO. Every adapter MUST emit a `NormalizationQualityReport` at `cio.metadata.normalization_quality` (see 04_INVESTIGATION_CONSTITUTION.md). X-Lab's Executive lens renders the coverage chip. | ⏳ Pending | Foundational P2 item · builds on P1-01 + P1-02. Adapter contract: `detect() + normalise() + quality_report()`. |
