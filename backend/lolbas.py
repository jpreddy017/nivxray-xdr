...
_L_DEFAULT: List[Dict[str, Any]] = [
    ...
    # ── Destructive wiper / anti-forensics precursor family (Feb-2026) ──
    {"bin": "wevtutil.exe", "argv": r"\bcl\b|clear-log|sl\s+\S+\s+/e:false",
     "purposes": ["Impact", "Defense Evasion"], "mitre": ["T1070.001"],
     "desc": "Windows event-log clearing / disabling (anti-forensics, wiper precursor)"},
    {"bin": "fsutil.exe", "argv": r"usn\s+deletejournal|file\s+setzerodata|volume\s+dismount",
     "purposes": ["Impact", "Defense Evasion"], "mitre": ["T1485", "T1070"],
     "desc": "USN journal wipe / file-content zeroing / volume dismount (destructive wiper)"},
    {"bin": "cipher.exe", "argv": r"/w:\S+|/w\s+\S+",
     "purposes": ["Impact"], "mitre": ["T1485"],
     "desc": "Cipher /w — overwrites free space on volume (secure erase, wiper)"},
